...
interface BulkUploadSalesGrowthProps {
	onSuccess?: (result: BulkResult) => void;
}

export default function BulkUploadSalesGrowth({
	onSuccess,
}: BulkUploadSalesGrowthProps) {
	const [file, setFile] = useState<File | null>(null);
	const [parsedData, setParsedData] = useState<BulkSalesGrowthData[]>([]);
			...
			if (fileExtension === 'csv') {
				Papa.parse(file, {
					...
					complete: (results) => {
						...
						if (errors.length === 0) {
							const processedData: BulkSalesGrowthData[] = data.map((row) => ({
				...
				reader.onload = (e) => {
					try {
						...
						if (errors.length === 0) {
							const processedData: BulkSalesGrowthData[] = jsonData.map(
								(row: any) => ({
	...
	const handleUpload = async () => {
		...
		setUploading(true);
		try {
			const response = await fetch('/api/sales-growth/bulk', {
				method: 'POST',
				headers: {
			...
			setUploadResult({
				success: [],
				errors: [
					{ data: {} as BulkSalesGrowthData, error: 'Network error occurred' },
				],
	...
	const downloadTemplate = () => {
		...
		link.download = 'sales_growth_template.csv';
		link.click();
	};
			...
			<CardContent>
				<Box sx={{ mb: 3 }}>
					<Typography variant="h6" gutterBottom>
						Bulk Upload Sales Growth
					</Typography>
					<Typography variant="body2" color="text.secondary">
						Upload CSV or Excel files to create multiple sales growth records at
						once
					</Typography>
				</Box>
